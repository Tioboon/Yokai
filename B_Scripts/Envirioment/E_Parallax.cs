...
public class E_Parallax : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        go_Floor = transform.Find("CloseObjects");
        if (go_Floor == null)
        {
            go_Floor = transform.Find("BackObjects");
            if (go_Floor == null)
            {
                go_Floor = transform.Find("BackTrees");
                if (go_Floor == null)
                {
                    go_Floor = transform.Find("Clouds");
                }
            }
        }
    ...
    private void GeneratePlainWorld(Vector2 screenSize, Vector2 objectSize, Vector2 initialPos)
    {
        ...
        {
            var newFloor = Instantiate(go_Floor, transform.parent, true);
            newFloor.name = "Floor --- " + nFloors;
