...
public class T_ShadowSpawn : MonoBehaviour
{
   public float limitToSpawnNewFloor;
    ...
    private void Start()
    {
        ...
        go_Floor = transform.Find("Shadow");
        ...
        var initPos = new Vector3(-worldSize.x / 2, yPos);
    ...
    private void GeneratePlainWorld(Vector2 screenSize, Vector2 objectSize, Vector2 initialPos)
    {
        ...
        for (int i = 0; i < floorsNumber; i++)
        {
            ...
            newFloor.name = "Shadow --- " + nFloors;
            newFloor.transform.position = initialPos + new Vector2(go_Floor.lossyScale.x * worldSpriteSize.x - 0.1f, 0) * i;
    ...
    private void CheckLastPosition(int nFloor)
    {
        ...
        {
            var initPos = new Vector2(floorObj.position.x, yPos);
            spawnChecked = false;
